...
def GnnNetsGR2valueFunc(gnnNets, target_idx):
    def value_func(batch):
        with torch.no_grad():
            result = gnnNets(batch)
            score = result[:, target_idx]
        return score

    return value_func


def GnnNetsNR2valueFunc(gnnNets_NR, node_idx, target_idx):
    def value_func(data):
        with torch.no_grad():
            results = gnnNets_NR(data)
            # select the corresponding node prob through the node idx on all the sampling graphs
            batch_size = data.batch.max() + 1
            results = results.reshape(batch_size, -1, results.shape[-1])
            score = results[:, node_idx, target_idx]
            return score
...
def gnn_score(coalition: list, data: Data, value_func: Callable[[Data], float],
...
def sparsity(coalition: list, data: Data, subgraph_building_method='zero_filling'):
    if subgraph_building_method == 'zero_filling':
        ...
        node_mask = torch.zeros(data.x.shape[0]).to(row.device)
