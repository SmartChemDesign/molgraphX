...
from .shapley import GnnNetsGC2valueFunc, GnnNetsNC2valueFunc, \
    gnn_score, mc_shapley, l_shapley, mc_l_shapley, NC_mc_l_shapley, sparsity, GnnNetsGR2valueFunc, GnnNetsNR2valueFunc
...
class MCTSNode(object):
    def __init__(self, coalition: list = None,
                 data: Data = None, ori_graph: nx.Graph = None,
...
class MCTS(object):
    ...
    Args:
        ...
        node_relation (:obj:`Callable[[int, int], bool]`, :obj:`None`):
        Boolean function with two arguments that must represent an equivalence relation on the nodes.
    ...
    def __init__(self, X: torch.Tensor, edge_index: torch.Tensor, num_hops: int,
                 node_relation: Callable[[int, int], bool] = None,
        ...
        graph_data = Data(x=self.X, edge_index=remove_self_loops(self.edge_index)[0] if len(
            self.edge_index) > 1 else self.edge_index)
        self.graph = to_networkx(graph_data, to_undirected=True)
        self.data = Batch.from_data_list([self.data])
        self.num_nodes = self.graph.number_of_nodes()
        if node_relation is None:
            self.node_relation = lambda x, y: x == y
        else:
    ...
    def mcts_rollout(self, tree_node):
        ...
        if len(tree_node.children) == 0:
            ...
            cur_full_subgraph = self.graph.subgraph(cur_graph_coalition)
            cur_subgraph = nx.quotient_graph(cur_full_subgraph, self.node_relation)

            # Current subgraph must contain at least two node classes
            if cur_subgraph.number_of_nodes() <= 1:
                return tree_node.P

            degree_list = [
                (cls, sum(1 for (u, v) in cur_full_subgraph.edges(cls) if (u in cls) != (v in cls)))
                for cls in cur_subgraph.nodes()
            ]
            degree_list = sorted(degree_list, key=lambda x: x[1], reverse=self.high2low)
            nodes_cls_list = [x[0] for x in degree_list]

            if self.new_node_idx:
                assert all(len(cls) == 1 for cls in nodes_cls_list)
                expand_nodes_cls_list = [
                    cls for cls in nodes_cls_list if self.new_node_idx not in cls
                ]
            else:
                expand_nodes_cls_list = nodes_cls_list

            if len(nodes_cls_list) > self.expand_atoms:
                expand_nodes_cls_list = expand_nodes_cls_list[:self.expand_atoms]

            for skip_cls in expand_nodes_cls_list:
                # for each nodes class, pruning it and get the remaining sub-graph
                # here we check the resulting sub-graphs and only keep the largest one
                coalition = [cls for cls in nodes_cls_list if cls != skip_cls]
                subgraphs = [
                    cur_subgraph.subgraph(c)
                    for c in nx.connected_components(cur_subgraph.subgraph(coalition))
                ]

                if self.new_node_idx:
                    for sub in subgraphs:
                        if any(self.new_node_idx in cls for cls in sub.nodes()):
                            main_sub = sub
                else:
                ...
                new_graph_coalition = sorted([
                    node for cls in main_sub.nodes() for node in cls
                ])

                # check the state map and merge the same sub-graph
                node_key = str(new_graph_coalition)
                if node_key in self.state_map:
                    new_node = self.state_map[node_key]
                else:
                    new_node = self.MCTSNodeClass(new_graph_coalition)
                    new_node.P = self.score_func(new_graph_coalition, new_node.data)
                    self.state_map[node_key] = new_node

                tree_node.children[node_key] = new_node

        sum_count = sum([c.N for c in tree_node.children.values()])
        selected_node = max(tree_node.children.values(), key=lambda x: x.Q() + x.U(sum_count))
...
class SubgraphX(object):
    ...
    Args:
        ...
        target_ids(:obj:`list`): indices of targets in the model output, which should be explained
        ...
        mode(:obj:`str`): Type of GCNN: eiether 'classification' or 'regression' (default: :obj:`classification`)
    ...
    Example:
        ...
        >>> subgraphx = SubgraphX(model=model, target_ids=2)
        >>> _, explanation_results, related_preds = subgraphx(x, edge_index)
    """

    def __init__(self, model, target_ids: list, device, num_hops: Optional[int] = None, verbose: bool = False,
                 explain_graph: bool = True, mode="classification", rollout: int = 20, min_atoms: int = 5,
                 c_puct: float = 10.0,
        ...
        if self.mode not in ["classification", "regression"]:
            raise ValueError(f"Invalid mode: {self.mode}. Valid values are 'regression', 'classification'")
    ...
    def get_mcts_class(self, x, edge_index,
                       node_relation: Callable[[int, int], bool],
                       node_idx: int = None,
    ...
    def explain(self, x: Tensor, edge_index: Tensor, label: int,
        ...
        if self.explain_graph:
            ...
            if value_func is None:
                if self.mode == "classification":
                    ...
                    value_func = GnnNetsGR2valueFunc(self.model, target_idx=label)

            if not saved_MCTSInfo_list:
                payoff_func = self.get_reward_func(value_func)
                self.mcts_state_map = self.get_mcts_class(x, edge_index, node_relation, score_func=payoff_func)
            ...
            if value_func is None:
                if self.mode == "classification":
                    ...
                    value_func = GnnNetsNR2valueFunc(self.model,
                                                     node_idx=self.mcts_state_map.new_node_idx,
                                                     target_idx=label)
        ...
        original_score = value_func(Data(x=x, edge_index=edge_index))
    ...
    def __call__(self, x: Tensor, edge_index: Tensor, **kwargs) \
        ...
        """
        node_relation = kwargs.get('node_relation')
        node_idx = kwargs.get('node_idx')
        ...
        # collect the target indices to be explained
        labels = tuple(torch.tensor([label]).to(self.device) for label in self.target_ids)
